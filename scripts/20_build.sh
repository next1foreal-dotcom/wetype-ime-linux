...
set -e
cd "$(dirname "$0")/.."
for tool in aarch64-linux-gnu-gcc patchelf; do
...
aarch64-linux-gnu-gcc -shared -fPIC -O2 -o runtime/libwetype-shim.so \
...
aarch64-linux-gnu-gcc -O0 -g -o harness/jinterop harness/jinterop.c \
  -ldl -lpthread -Wl,--no-as-needed -lm "$PWD/runtime/libz.so.1"
aarch64-linux-gnu-gcc -O0 -g -o harness/probe harness/probe.c -ldl -lpthread \
